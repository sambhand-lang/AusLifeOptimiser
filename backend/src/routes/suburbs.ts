...
router.get('/ssc/:ssc/details', async (req: Request, res: Response) => {
  try {
    const ssc = req.params.ssc;
    if (!ssc) return res.status(400).json({ error: 'SSC is required' });

    const result = await query('SELECT * FROM suburbs WHERE ssc = ? LIMIT 1', [ssc]);
    ...
    const realData: SuburbRealData = await ExternalDataService.getSuburbRealData(
      suburb.suburb_name,
      suburb.state,
      suburb.postcode
    );
    ...
    const commuteObj = (realData as any)?.commute;
    if (commuteObj && typeof commuteObj === 'object' && commuteObj.drivingTimeMinutes && commuteObj.drivingTimeMinutes.value != null) {
      normalized.commute = { drivingTimeMinutes: commuteObj.drivingTimeMinutes };
      if (commuteObj.drivingTimeMinutes.source) sources.push(commuteObj.drivingTimeMinutes.source);
    }
    if ((realData as any)?.schools && (realData as any).schools.count && (realData as any).schools.count.value != null) {
      normalized.schools = { count: (realData as any).schools.count };
      if ((realData as any).schools.count.source) sources.push((realData as any).schools.count.source);
    }
    if ((realData as any).publicTransportStops && (realData as any).publicTransportStops.value != null) {
      normalized.publicTransportStops = (realData as any).publicTransportStops;
      if ((realData as any).publicTransportStops.source) sources.push((realData as any).publicTransportStops.source);
    }
    if ((realData as any).parks && (realData as any).parks.value != null) {
      normalized.parks = (realData as any).parks;
      if ((realData as any).parks.source) sources.push((realData as any).parks.source);
    }
