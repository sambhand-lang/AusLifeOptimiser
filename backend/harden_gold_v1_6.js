...
console.log('--- FINAL GOLD HARDENING v1.6 ---');
...
db.serialize(() => {
    ...
    `, (err, rows) => {
        ...
        console.log(`Auditing ${rows.length} suburbs for Final Gold v1.6 compliance...`);
        let count = 0;
        ...
        rows.forEach(r => {
            ...
            if (name.includes('beach') || name.includes('ocean')) lifestyle += 15;
            if (income > 3000 && price > 1600000) lifestyle += 10;
            
            // Premium Coastal Floor
            if ((name.includes('swanbourne') || name.includes('cottesloe')) && lifestyle < 82) lifestyle = 82;
            ...
            // SPECIAL TUNING
            if (lifestyle >= 90) baseScore += 2.5;
            if (price > 1800000 && lifestyle > 75 && schoolsScore > 75) baseScore += 3.5;

            // PENALTIES
            if (employment > 85 && lifestyle < 40 && pop < 15000) baseScore -= 5; // Mining Penalty
            if (pop < 3000) baseScore -= 5;

            // SCORE EXPANSION
            let finalOverall = baseScore;
            if (baseScore > 75) finalOverall += (baseScore - 75) * 0.4;
            if (baseScore < 70) finalOverall -= (70 - baseScore) * 0.4;
        ...
        console.log(`Final Gold v1.6 Complete. ${count} suburbs recalibrated.`);
        db.close();
    });
});
