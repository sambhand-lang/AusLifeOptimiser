...
async function upsertRow(r) {
    ...
    if (!suburb && postcode) {
        const suburbRes = await (0, db_1.query)('SELECT suburb_name, state FROM suburbs WHERE postcode = ? LIMIT 1', [postcode]);
    ...
    const updateRes = await (0, db_1.query)(updateSql, [houseNum, rentNum, state, source, suburb.toUpperCase(), postcode]);
    if (updateRes.rowCount === 0) {
        await (0, db_1.query)(`INSERT INTO property_data (suburb_name, postcode, state, median_house_price, median_rent, source) VALUES (?, ?, ?, ?, ?, ?)`, [suburb, postcode, state, houseNum, rentNum, source]);
    }
}
...
async function main() {
    ...
    const filePath = path_1.default.isAbsolute(arg) ? arg : path_1.default.join(process.cwd(), arg);
    await importCsv(filePath);
    process.exit(0);
}
main().catch(err => { console.error(err); process.exit(1); });
//# sourceMappingURL=importPropertyData.js.map