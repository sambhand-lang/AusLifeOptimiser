...
async function importSuburbs() {
    try {
        ...
        const response = await axios_1.default.get('https://raw.githubusercontent.com/michalsn/australian-suburbs/master/json/suburbs.json');
        const suburbs = response.data;
        ...
        for (const suburb of suburbs) {
            ...
            await (0, db_1.query)(`INSERT INTO suburbs (suburb_name, postcode, state, city, latitude, longitude)
         VALUES (?, ?, ?, ?, ?, ?)`, [
                suburb.suburb,
                suburb.postcode,
                stateCode,
        ...
        console.log('\nSummary by state:');
        const stateResult = await (0, db_1.query)(`SELECT state, COUNT(*) as count FROM suburbs GROUP BY state ORDER BY state`);
        for (const row of stateResult.rows) {
            console.log(`  ${row.state}: ${row.count} suburbs`);
        }
        // Verify counts by city
        console.log('\nSummary by city:');
        const cityResult = await (0, db_1.query)(`SELECT city, COUNT(*) as count FROM suburbs GROUP BY city ORDER BY count DESC LIMIT 10`);
...
main();
//# sourceMappingURL=importSuburbs.js.map