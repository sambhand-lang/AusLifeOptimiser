...
console.log('--- GOLDEN SYNC v1.7 ULTIMATE ---');
...
db.serialize(() => {
    ...
    `, (err, rows) => {
        ...
        console.log(`Syncing ${rows.length} suburbs from RAW data...`);
        let count = 0;
        ...
        rows.forEach(r => {
            ...
            let barrierScore = housePrice < 600000 ? 100 : housePrice < 1000000 ? 80 : housePrice < 2000000 ? 60 : housePrice < 3000000 ? 40 : housePrice < 4500000 ? 25 : 10;
            ...
            // 4. FAMILY/SCHOOLS (Family Score includes Parks)
            ...
            // 5. LIFESTYLE (Recalculated from densities)
            const cafeCount = cafes + restaurants;
            const amenityDensityScore = Math.min(100, (cafeCount / effectivePop) * 12500);
            const recScore = Math.min(100, (parks / effectivePop) * 5000); // Recreation proxy
        ...
        console.log(`Golden Sync Complete. ${count} suburbs synchronized with 1:1 Engine Logic.`);
        db.close();
    });
});
