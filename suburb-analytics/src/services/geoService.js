...
import { enforceIntegrity } from '../utils/integrityGuard.js';

let boundaries = null;
try {
  // Try a simple relative path first (works when running from project root)
  boundaries = JSON.parse(fs.readFileSync('./data/abs/suburb_boundaries.geojson', 'utf8'));
} catch (e) {
  try {
    // Fallback using import.meta.url to construct a path relative to this module
...
export async function generateSuburbMetrics(suburbName) {
  let polygon = null;
  let centroid = null;
  ...
  if (boundaries && Array.isArray(boundaries.features)) {
    const feature = boundaries.features.find(f => {
      ...
      if (!p) return false;
      // try common property names then fallback to sensible casing
  ...
  // fallback to census centroid data, or use hardcoded Sydney CBD for demo fallback
  ...
  // If still no centroid, use a reasonable Sydney-area fallback (Sydney CBD)
  if (!centroid) {
  ...
  return {
    suburb: suburbName,
    metrics: enforceIntegrity(metrics)
  };
}
